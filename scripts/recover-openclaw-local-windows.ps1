...
if ([string]::IsNullOrWhiteSpace($ArchivePath)) {
  if (-not [string]::IsNullOrWhiteSpace($env:RECLAW_BACKUP_DIR)) {
    $ArchivePath = Join-Path $env:RECLAW_BACKUP_DIR 'backup.zip'
  } elseif (-not [string]::IsNullOrWhiteSpace($env:BACKUP_DIR)) {
    $ArchivePath = Join-Path $env:BACKUP_DIR 'backup.zip'
  } else {
    $ArchivePath = Join-Path $WorkspaceRoot 'claw-backup\backup.zip'
  }
}
...
try {
  $restoreArgs = @('bin/cli.js', 'restore')
  if (-not [string]::IsNullOrWhiteSpace($Password)) {
    $restoreArgs += @('--password', $Password)
  }
  $restoreArgs += $ArchivePath
  Invoke-ExternalCommand -FilePath 'node' -Arguments $restoreArgs -StepName 'Restore backup'
  Invoke-ExternalCommand -FilePath 'node' -Arguments @('scripts/verify-openclaw-state.js', 'restored') -StepName 'Verify restored payload'
} finally {
  Pop-Location
}
...
if (!cfg.gateway.mode) {
  ...
  console.log('[recover-win] Set gateway.mode=local');
}
"@
