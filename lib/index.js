...
class BackupService {
  ...
  trySevenZipExtract(archivePath, destDir, password) {
    ...
    for (const cmd of cmds) {
      try {
        ...
        const result = spawnSync(cmd, args, { stdio: 'pipe', encoding: 'utf-8', windowsHide: true });
  ...
  async restore(archivePath, options = {}) {
      ...
      if (archiveType === 'zip') {
        ...
        if (!admZipOk) {
          ...
          if (!zipIsEncrypted && process.platform !== 'win32') {
            ...
            try {
              const stdioLevel = isDebugEnabled() ? 'inherit' : 'ignore';
    ...
    if (shouldRestoreWorkspace) {
      if (!isDebugEnabled() && !this._printedPrepareMsg) {
      ...
      if (!(await fs.pathExists(path.join(this.home, 'node_modules')))) {
          if (await fs.pathExists(path.join(this.home, 'package.json'))) {
              if (isDebugEnabled()) console.log('   📦 node_modules missing. Running npm install...');
              try {
                  const stdioLevel = isDebugEnabled() ? 'inherit' : 'ignore';
                  execSync('npm install', { cwd: this.home, stdio: stdioLevel });
              } catch (e) {
                  if (isDebugEnabled()) console.warn('   ⚠️ npm install failed. You may need to run it manually.');
              }
          }
      }
    }
    ...
    // Ensure gateway auth token exists in restored config (tests expect it; OpenClaw often relies on it)
    try {
      const configPath = path.join(this.home, 'openclaw.json');
      if (await fs.pathExists(configPath)) {
        const configData = await fs.readJson(configPath);
        ...
        if (!gw.auth.token) {
          ...
          configData.gateway = gw;
          await fs.writeJson(configPath, configData, { spaces: 2 });
        }
      }
    } catch (_) {
      // best effort only; ignore if config missing or unreadable
    }
    ...
    const shouldPrintDashboard =
      process.env.RECLAW_PRINT_DASHBOARD_URL === '1' ||
      process.env.RECLAW_PRINT_DASHBOARD_URL === 'true';
  ...
  async ensureDashboardTokenAndUrl() {
    ...
    const result = spawnSync(process.execPath, [helperPath, '--home', this.home], {
      encoding: 'utf-8',
      timeout: 15000,
      stdio: isDebugEnabled() ? 'inherit' : 'pipe',
      windowsHide: true,
    });
  ...
  async postRestoreRuntimeCheck() {
    ...
    if (doctor.status !== 0 && isDebugEnabled()) {
